...
public class AESCrypt {
	...
	public void encrypt(int version, VFS4JPathName fromPath, VFS4JPathName toPath)
			throws IOException, GeneralSecurityException {
		InputStream in = null;
		OutputStream out = null;
		try {
			in = new BufferedInputStream(VFS4JFiles.newInputStream(fromPath));
			debug("Opened for reading: " + fromPath);
			out = new BufferedOutputStream(VFS4JFiles.newOutputStream(toPath));
			debug("Opened for writing: " + toPath);
	...
	public void decrypt(VFS4JPathName fromPath, VFS4JPathName toPath)
			throws IOException, GeneralSecurityException {
		InputStream in = null;
		OutputStream out = null;
		try {
			...
			decrypt(VFS4JFiles.size(fromPath), in, out);
		} finally {
			if (in != null) {
				in.close();
			}
